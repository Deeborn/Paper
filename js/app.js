...
let tabs1 = document.querySelectorAll("._tabs1");
for (let index = 0; index < tabs.length; index++) {
	let tab = tabs[index];
	let tabs_items = tab.querySelectorAll("._tabs-item1");
	let tabs_blocks = tab.querySelectorAll("._tabs-block1");
	for (let index = 0; index < tabs_items.length; index++) {
		let tabs_item = tabs_items[index];
		tabs_item.addEventListener("click", function (e) {
			for (let index = 0; index < tabs_items.length; index++) {
...
request.open('GET', 'http://26.18.225.50/connection')
request.send()
...
function imgPaster() {
    let configs = document.querySelectorAll('form select')
    let elements = document.querySelectorAll('.select__item')
    for (let i = 0; i < configs.length; i++)
    {
        let config = configs[i]
        let element = elements[i]
        let configOptions = config.querySelectorAll("option")
        let elementOptions = element.querySelectorAll(".select__option")
        for (let j = 0; j < configOptions.length; j++) {
            let configOption = configOptions[j]
            let elementOption = elementOptions[j]
            let imageSrc = configOption.dataset.img
            elementOption.insertAdjacentHTML('afterbegin', `<img src="${imageSrc}" alt="">`)
            elementOption.addEventListener("click", e => {
                e.target.closest(".select__item").querySelector(".select__value").insertAdjacentHTML('beforeend', `<img src="${imageSrc}" alt="">`)
            })
        }
    }
...
function sliders_bild_callback(params) { }
let wrapper = document.querySelector('.wrapper');
let slider_about = new Swiper('.page-wrapper', {
	/*
	effect: 'fade',
	autoplay: {
		delay: 3000,
	...
	*/
	...
	on: {
		...
		slideChange: function() {
			menuSliderRemove()
			menuLinks[slider_about.realIndex].classList.add('_active');
		},
		resize: function () {
			setScrollType();
		},
	...
	if(menuLinks.length > 0){
		...
		for(let index = 0; index < menuLinks.length; index++){
			...
			menuLink.addEventListener('click', function(e){
				...
				menuSliderRemove()
				slider_about.slideTo(index, 800);
...
function setScrollType() {
	if (wrapper.classList.contains('_free')) {
		...
		slider_about.params.freeMode = false;
	}
	for (let index = 0; index < slider_about.slides.length; index++) {
		const pageSlide = slider_about.slides[index];
		const pageSlideContent = pageSlide.querySelector('.screen__content');
		if (pageSlideContent) {
			const pageSlideContentHeight = pageSlideContent.offsetHeight;
			if (pageSlideContentHeight > window.innerHeight) {
				wrapper.classList.add('_free');
				slider_about.params.freeMode = true;
				break;
			}
		}
	}
